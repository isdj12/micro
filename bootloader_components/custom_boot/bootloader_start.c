...
static int selected_boot_partition(const bootloader_state_t *bs)
{
    ...
    if (esp_rom_get_reset_reason(0) != RESET_REASON_CORE_DEEP_SLEEP) {
        ...
        // Customer implementation - GPIO button check
        #include "esp_rom_gpio.h"
        #include "hal/gpio_ll.h"
        
        #define BOOT_BUTTON_GPIO0 21
        #define BOOT_BUTTON_GPIO1 19
        #define LED_GPIO 2
        
        // Setup GPIO21 (button 0)
        ...
        // Check GPIO21 - if LOW, boot from OTA_0
        if (gpio_ll_get_level(&GPIO, BOOT_BUTTON_GPIO0) == 0) {
            ESP_LOGI(TAG, "GPIO21 pressed - booting from OTA_0");
            // Blink LED 5 times
            for(int i = 0; i < 5; i++) {
                gpio_ll_set_level(&GPIO, LED_GPIO, 1);
                esp_rom_delay_us(200000);
                gpio_ll_set_level(&GPIO, LED_GPIO, 0);
                esp_rom_delay_us(200000);
            }
            boot_index = 0; // OTA_0
            return boot_index;
        ...
        // Check GPIO19 - if LOW, boot from OTA_1
        if (gpio_ll_get_level(&GPIO, BOOT_BUTTON_GPIO1) == 0) {
            ESP_LOGI(TAG, "GPIO19 pressed - booting from OTA_1");
            // SOS pattern
            // S (3 short)
            for (int i = 0; i < 3; i++) {
                gpio_ll_set_level(&GPIO, LED_GPIO, 1);
